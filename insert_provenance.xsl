...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:foaf="http://xmlns.com/foaf/0.1/"
    ...
    xmlns:xsd="http://www.w3.org/2001/XMLSchema#" xmlns:kon="http://kerameikos.org/ontology#" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:org="http://www.w3.org/ns/org#"
        ...
        <xsl:element name="rdf:RDF">
            ...
            <dcterms:ProvenanceStatement rdf:about="{concat($uri, '#provenance')}">
                ...
                <xsl:variable name="creation">
                    <xsl:choose>
                        <xsl:when test="document('xhtml-modifications.xml')//file[@id = $id]">
                            <xsl:value-of select="document('xhtml-modifications.xml')//file[@id = $id]/date[last()]"/>
                        </xsl:when>
                        <xsl:when test="document('rdf-modifications.xml')//file[@id = $id]">
                            <xsl:value-of select="document('rdf-modifications.xml')//file[@id = $id]/date[last()]"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:variable>
                ...
                <prov:wasGeneratedBy>
                    <prov:Activity>
                        ...
                        <xsl:choose>
                            <xsl:when test="contains($creation, '2012-10-28')">
